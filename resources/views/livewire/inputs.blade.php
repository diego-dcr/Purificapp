<div class="flex w-full flex-1 flex-col gap-6" wire:poll.5s>
    <section class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <article class="rounded-xl border border-neutral-200 bg-white p-5 shadow-sm dark:border-neutral-700 dark:bg-zinc-900">
            ...
            <p class="mt-3 text-3xl font-semibold text-zinc-900 dark:text-zinc-50">{{ count($this->inputs) }}</p>
        ...
        <article class="rounded-xl border border-neutral-200 bg-white p-5 shadow-sm dark:border-neutral-700 dark:bg-zinc-900">
            ...
            <p class="mt-3 text-3xl font-semibold text-zinc-900 dark:text-zinc-50">{{ $this->inputs->sum('waterjug_count') }}</p>
    ...
    <section class="grid gap-6 xl:grid-cols-[minmax(0,1.9fr)_minmax(320px,0.9fr)]">
        <div class="rounded-xl border border-neutral-200 bg-white shadow-sm dark:border-neutral-700 dark:bg-zinc-900">
            <div class="flex flex-col gap-4 border-b border-neutral-200 px-6 py-5 dark:border-neutral-700 md:flex-row md:items-end md:justify-between">
                <div>
                    <h2 class="text-lg font-semibold text-zinc-900 dark:text-zinc-50">Gestión de entregas/ventas (Livewire)</h2>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-neutral-200 text-sm dark:divide-neutral-700">
                    ...
                    <tbody class="divide-y divide-neutral-200 dark:divide-neutral-700">
                        @forelse ($this->inputs as $inputItem)
                            <tr>
                                ...
                                <td class="px-6 py-4 align-top">
                                    <div class="flex justify-end gap-2">
                                        <flux:button type="button" icon="pencil-square" size="sm" variant="filled" class="bg-zinc-500 hover:bg-zinc-600" wire:click="edit({{ $inputItem->id }})">
                                            Editar
                                        </flux:button>
                                           <flux:button type="button" icon="eye" size="sm" variant="primary" color="lime" wire:click="showDetails({{ $inputItem->id }})">
                                               Ver detalles
                                           </flux:button>
                                        <flux:button type="button" icon="trash" size="sm" variant="danger" wire:click="delete({{ $inputItem->id }})" wire:confirm="¿Seguro que deseas eliminar este registro?">
                                            Eliminar
                                        </flux:button>
                                    </div>
                                </td>
        ...
        <aside class="rounded-xl border border-neutral-200 bg-white p-5 dark:border-neutral-700 dark:bg-zinc-900">
            <div>
                <flux:heading size="lg">{{ $editingInputId ? 'Editar registro' : 'Nuevo registro' }}</flux:heading>
                <flux:text class="mt-1">{{ $editingInputId ? 'Actualiza los datos de la entrega/venta.' : 'Registra una nueva entrega/venta y sus garrafones.' }}</flux:text>
            </div>
            ...
            @else
                <form wire:submit="save" class="mt-6 space-y-5">
                    ...
                    <div class="flex justify-end gap-3">
                        ...
                        <flux:button variant="primary" color="sky" size="sm" type="submit">{{ $editingInputId ? 'Actualizar registro' : 'Guardar registro' }}</flux:button>
                    </div>
                </form>
            @endif
        </aside>
        ...
        {{-- Modal de detalles --}}
        @if ($showDetailsModal && $detailsInput)
            <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
                <div class="w-full max-w-lg rounded-xl bg-white p-6 shadow-xl dark:bg-zinc-900">
                    ...
                    <div class="space-y-2 text-sm">
                        <div><strong>Cliente:</strong> {{ $detailsInput->customer->name }} ({{ $detailsInput->customer->number }})</div>
                        <div><strong>Concepto:</strong> {{ $detailsInput->concept->name }} ({{ $detailsInput->concept->code }})</div>
                        <div><strong>Costo:</strong> ${{ number_format((float) $detailsInput->cost, 2) }}</div>
                        <div><strong>Garrafones:</strong> {{ $detailsInput->waterjug_count }}</div>
                        <div><strong>Fecha:</strong> {{ $detailsInput->timestamp->format('Y-m-d H:i') }}</div>
                        <div><strong>Usuario:</strong> {{ $detailsInput->user->name }}</div>
                        <div><strong>Ruta:</strong> {{ $detailsInput->route?->name ?? '-' }}</div>
                        <div><strong>Códigos de garrafones:</strong>
                            <ul class="list-disc ml-6">
                                @if (!empty($detailsInput->waterjugSales))
                                    @foreach ($detailsInput->waterjugSales as $jug)
                                        <li>{{ $jug->waterjug_codebar }}</li>
                                    @endforeach
                                @else
