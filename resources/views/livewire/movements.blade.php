            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-neutral-200 text-sm dark:divide-neutral-700">
                    ...
                    <tbody class="divide-y divide-neutral-200 dark:divide-neutral-700">
                        @forelse ($this->inputs as $item)
                            <tr>
                                <td class="px-6 py-4">#{{ $item->id }}</td>
                                <td class="px-6 py-4">{{ $item->customer->name ?? '—' }}</td>
                                <td class="px-6 py-4 text-center">{{ $item->waterjug_count }}</td>
                                <td class="px-6 py-4">{{ $item->timestamp?->format('Y-m-d H:i') }}</td>
                                <td class="px-6 py-4 text-right">
                                    <flux:button type="button" size="sm" icon="eye" variant="primary" color="lime" wire:click="showInputDetails({{ $item->id }})">Ver detalles</flux:button>
                                </td>
                            </tr>
                        @empty
                            <tr><td colspan="5" class="px-6 py-8 text-center text-zinc-500 dark:text-zinc-400">No hay inputs registrados.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-neutral-200 text-sm dark:divide-neutral-700">
                    ...
                    <tbody class="divide-y divide-neutral-200 dark:divide-neutral-700">
                        ...
                        @empty
                            <tr><td colspan="5" class="px-6 py-8 text-center text-zinc-500 dark:text-zinc-400">No hay outputs registrados.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
    ...
    @if ($this->selectedInput || $this->selectedOutput)
        <section class="rounded-xl border border-neutral-200 bg-white p-5 shadow-sm dark:border-neutral-700 dark:bg-zinc-900">
            ...
            @if ($this->selectedInput)
                <p class="mb-3 text-sm text-zinc-500 dark:text-zinc-400">Input #{{ $this->selectedInput->id }} · Fecha: {{ $this->selectedInput->timestamp?->format('Y-m-d H:i') }}</p>
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-neutral-200 text-sm dark:divide-neutral-700">
                        <thead class="bg-zinc-50 dark:bg-zinc-800/70">
                            <tr class="text-left text-zinc-500 dark:text-zinc-400">
                        ...
                        <tbody class="divide-y divide-neutral-200 dark:divide-neutral-700">
                            @forelse ($this->selectedInput->waterjugSales as $wj)
                                <tr>
                                    <td class="px-6 py-4">{{ $wj->waterjug_codebar }}</td>
            ...
            @if ($this->selectedOutput)
                <p class="mb-3 text-sm text-zinc-500 dark:text-zinc-400">Output #{{ $this->selectedOutput->id }} · Fecha: {{ $this->selectedOutput->timestamp?->format('Y-m-d H:i') }}</p>
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-neutral-200 text-sm dark:divide-neutral-700">
                        <thead class="bg-zinc-50 dark:bg-zinc-800/70">
                            <tr class="text-left text-zinc-500 dark:text-zinc-400">
                        ...
                        <tbody class="divide-y divide-neutral-200 dark:divide-neutral-700">
                            @forelse ($this->selectedOutput->waterjugOutputs as $wj)
                                <tr>
                                    <td class="px-6 py-4">{{ $wj->waterjug_codebar }}</td>
                                    <td class="px-6 py-4">{{ $wj->timestamp?->format('Y-m-d H:i') }}</td>
                                </tr>
