<div class="flex w-full flex-1 flex-col gap-6" wire:poll.5s>
                    ...
                    <tbody class="divide-y divide-neutral-200 dark:divide-neutral-700">
                        @forelse ($this->retornos as $item)
                            <tr>
                                ...
                                <td class="px-6 py-4">{{ $item->timestamp?->format('Y-m-d H:i') }}</td>
                                <td class="px-6 py-4 text-right">
                                    <flux:button type="button" size="sm" icon="eye" variant="primary" color="lime" wire:click="showRetornoDetails({{ $item->id }})">Ver detalles</flux:button>
                                </td>
                            </tr>
                        @empty
    ...
    @if ($this->selectedSale || $this->selectedRetorno)
        <section class="rounded-xl border border-neutral-200 bg-white p-5 shadow-sm dark:border-neutral-700 dark:bg-zinc-900">
            ...
            @if ($this->selectedRetorno)
                <p class="mb-3 text-sm text-zinc-500 dark:text-zinc-400">Retorno #{{ $this->selectedRetorno->id }} · Fecha: {{ $this->selectedRetorno->timestamp?->format('Y-m-d H:i') }}</p>
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-neutral-200 text-sm dark:divide-neutral-700">
                        <thead class="bg-zinc-50 dark:bg-zinc-800/70">
                            <tr class="text-left text-zinc-500 dark:text-zinc-400">
                        ...
                        <tbody class="divide-y divide-neutral-200 dark:divide-neutral-700">
                            @forelse ($this->selectedRetorno->carboyRetornos as $wj)
                                <tr>
                                    <td class="px-6 py-4">{{ $wj->carboy_codebar }}</td>
                                    <td class="px-6 py-4">{{ $wj->timestamp?->format('Y-m-d H:i') }}</td>
                                </tr>
