<x-layouts::app :title="__('Salidas')">
        ...
        <section class="grid gap-6 xl:grid-cols-[minmax(0,1.6fr)_minmax(320px,0.9fr)]">
            <div
                ...
                <div
                    ...
                    <div>
                        <h2 class="text-lg font-semibold text-zinc-900 dark:text-zinc-50">Gestión de salidas</h2>
                        <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Administra el registro de salidas de garrafas por usuario.</p>
                    </div>

                    <div class="flex flex-col items-start gap-3 sm:flex-row sm:items-center">
                        <flux:button type="button" variant="primary" color="sky" size="sm" icon="plus"
                            onclick="window.location.href='{{ route('outputs.index') }}'">
                            Nueva salida
                        </flux:button>
                    </div>
                </div>
                                    ...
                                    <td class="px-6 py-4 align-top">
                                        <div class="flex justify-end gap-2">
                                            ...
                                            <form method="POST" action="{{ route('outputs.destroy', $outputItem) }}"
                                                onsubmit="return confirm('¿Seguro que deseas eliminar esta salida?');">
                                                @csrf
                                                @method('DELETE')
            ...
            <aside class="rounded-xl border border-neutral-200 bg-white p-5 dark:border-neutral-700 dark:bg-zinc-900">
                <div>
                    ...
                    <flux:text class="mt-1">
                        ...
                        @else
                            Registra una nueva salida de garrafas.
                        @endisset
                    </flux:text>
                </div>

                <form method="POST"
                    action="@isset($output){{ route('outputs.update', $output) }}@else{{ route('outputs.store') }}@endisset"
                    ...
                    <div>
                        ...
                        <select name="user_id" required
                            ...
                            @foreach ($users as $user)
                                <option value="{{ $user->id }}" @if (isset($output) && $output->user_id === $user->id) selected @endif>
                                    {{ $user->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="route_id"
                            ...
                            @foreach ($routes as $route)
                                <option value="{{ $route->id }}" @if (isset($output) && $output->route_id === $route->id) selected @endif>
