<x-layouts::app :title="__('Detalle de Retorno')">
    <div class="flex w-full flex-1 flex-col gap-6">
        <div class="rounded-xl border border-neutral-200 bg-white p-6 shadow-sm dark:border-neutral-700 dark:bg-zinc-900">
            <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                <div>
                    <h2 class="text-xl font-semibold text-zinc-900 dark:text-zinc-50">Detalle de movimiento de retorno #{{ $retorno->id }}</h2>
                    <p class="text-sm text-zinc-500 dark:text-zinc-400">Información completa del retorno y códigos de garrafón ligados.</p>
                </div>
            ...
            <div class="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                <article class="rounded-lg border border-neutral-200 p-4 dark:border-neutral-700">
                    ...
                    <p class="mt-1 text-sm font-semibold text-zinc-900 dark:text-zinc-50">{{ $retorno->user->name ?? '—' }}</p>
                </article>
                <article class="rounded-lg border border-neutral-200 p-4 dark:border-neutral-700">
                    <p class="text-xs text-zinc-500 dark:text-zinc-400">Ruta</p>
                    <p class="mt-1 text-sm font-semibold text-zinc-900 dark:text-zinc-50">{{ $retorno->route->name ?? '—' }}</p>
                </article>
                <article class="rounded-lg border border-neutral-200 p-4 dark:border-neutral-700">
                    <p class="text-xs text-zinc-500 dark:text-zinc-400">Fecha de creación</p>
                    <p class="mt-1 text-sm font-semibold text-zinc-900 dark:text-zinc-50">{{ $retorno->timestamp?->format('Y-m-d H:i') }}</p>
                </article>
                <article class="rounded-lg border border-neutral-200 p-4 dark:border-neutral-700">
        ...
        <div class="rounded-xl border border-neutral-200 bg-white shadow-sm dark:border-neutral-700 dark:bg-zinc-900">
            <div class="border-b border-neutral-200 px-6 py-5 dark:border-neutral-700">
                <h3 class="text-lg font-semibold text-zinc-900 dark:text-zinc-50">Códigos de garrafón ligados al retorno</h3>
            </div>
                        ...
                        @empty
                            <tr>
                                <td colspan="2" class="px-6 py-8 text-center text-zinc-500 dark:text-zinc-400">
                                    Este retorno no tiene códigos de garrafón ligados.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
