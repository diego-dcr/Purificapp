<x-layouts::app :title="__('Conceptos')">
    <div class="flex w-full flex-1 flex-col gap-6">

        <section class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            <article
                class="rounded-xl border border-neutral-200 bg-white p-5 shadow-sm dark:border-neutral-700 dark:bg-zinc-900">
                <p class="text-sm text-zinc-500 dark:text-zinc-400">Conceptos registrados</p>
                <p class="mt-3 text-3xl font-semibold text-zinc-900 dark:text-zinc-50">{{ count($concepts) }}</p>
                <p class="mt-1 text-sm text-emerald-600 dark:text-emerald-400">Total en el sistema</p>
            </article>

            <article
                class="rounded-xl border border-neutral-200 bg-white p-5 shadow-sm dark:border-neutral-700 dark:bg-zinc-900">
                <p class="text-sm text-zinc-500 dark:text-zinc-400">Últimas actualizaciones</p>
                <p class="mt-3 text-3xl font-semibold text-zinc-900 dark:text-zinc-50">
                    @if ($concepts->isNotEmpty())
                        {{ $concepts->sortByDesc('updated_at')->first()->updated_at->format('d M') }}
                    @else
                        —
                    @endif
                </p>
                <p class="mt-1 text-sm text-sky-600 dark:text-sky-400">Registro más reciente</p>
            </article>
        </section>

        <section class="grid gap-6 xl:grid-cols-[minmax(0,1.6fr)_minmax(320px,0.9fr)]">
            <div
                class="rounded-xl border border-neutral-200 bg-white shadow-sm dark:border-neutral-700 dark:bg-zinc-900">
                <div
                    class="flex flex-col gap-4 border-b border-neutral-200 px-6 py-5 dark:border-neutral-700 md:flex-row md:items-end md:justify-between">
                    <div>
                        <h2 class="text-lg font-semibold text-zinc-900 dark:text-zinc-50">Gestión de conceptos</h2>
                        <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Define los conceptos disponibles para transacciones y movimientos.</p>
                    </div>

                    <div class="flex flex-col items-start gap-3 sm:flex-row sm:items-center">
                        <flux:button type="button" variant="primary" color="sky" size="sm" icon="plus"
                            onclick="window.location.href='{{ route('concepts.index') }}'">
                            Nuevo concepto
                        </flux:button>
                    </div>
                </div>

                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-neutral-200 text-sm dark:divide-neutral-700">
                        <thead class="bg-zinc-50 dark:bg-zinc-800/70">
                            <tr class="text-left text-zinc-500 dark:text-zinc-400">
                                <th class="px-6 py-3 font-medium">Nombre</th>
                                <th class="px-6 py-3 font-medium">Código</th>
                                <th class="px-6 py-3 font-medium">Actualizado</th>
                                <th class="px-6 py-3 font-medium text-right">Acciones</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-neutral-200 dark:divide-neutral-700">
                            @forelse ($concepts as $conceptItem)
                                <tr>
                                    <td class="px-6 py-4 font-medium text-zinc-900 dark:text-zinc-50">
                                        {{ $conceptItem->name }}
                                    </td>
                                    <td class="px-6 py-4 text-zinc-600 dark:text-zinc-300">
                                        <code class="text-xs font-mono bg-zinc-100 px-2 py-1 rounded dark:bg-zinc-800">{{ $conceptItem->code }}</code>
                                    </td>
                                    <td class="px-6 py-4 text-xs text-zinc-500 dark:text-zinc-400">
                                        {{ $conceptItem->updated_at->format('d M Y, H:i') }}
                                    </td>
                                    <td class="px-6 py-4 align-top">
                                        <div class="flex justify-end gap-2">
                                            <flux:button type="button" icon="pencil-square" size="sm" variant="filled"
                                                class="bg-zinc-500 hover:bg-zinc-600"
                                                onclick="window.location.href='{{ route('concepts.edit', $conceptItem) }}'">
                                                Editar
                                            </flux:button>

                                            <form method="POST" action="{{ route('concepts.destroy', $conceptItem) }}"
                                                onsubmit="return confirm('¿Seguro que deseas eliminar este concepto?');">
                                                @csrf
                                                @method('DELETE')
                                                <flux:button type="submit" icon="trash" size="sm" variant="danger">
                                                    Eliminar
                                                </flux:button>
                                            </form>
                                        </div>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="4" class="px-6 py-8 text-center text-zinc-500 dark:text-zinc-400">
                                        No hay conceptos registrados.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
            </div>

            <aside class="rounded-xl border border-neutral-200 bg-white p-5 dark:border-neutral-700 dark:bg-zinc-900">
                <div>
                    <flux:heading size="lg">
                        @isset($concept)
                            Editar concepto
                        @else
                            Nuevo concepto
                        @endisset
                    </flux:heading>
                    <flux:text class="mt-1">
                        @isset($concept)
                            Modifica los datos del concepto.
                        @else
                            Define un nuevo concepto con código único.
                        @endisset
                    </flux:text>
                </div>

                <form method="POST"
                    action="@isset($concept){{ route('concepts.update', $concept) }}@else{{ route('concepts.store') }}@endisset"
                    class="mt-6 space-y-5">
                    @csrf
                    @isset($concept)
                        @method('PUT')
                    @endisset

                    <flux:input name="name" label="Nombre" type="text" required
                        placeholder="Venta de agua" value="{{ isset($concept) ? $concept->name : '' }}" />

                    <flux:input name="code" label="Código" type="text" required
                        placeholder="VENTA" value="{{ isset($concept) ? $concept->code : '' }}" />

                    <div class="flex justify-end gap-3">
                        <flux:button type="button" size="sm" variant="danger"
                            onclick="window.location.href='{{ route('concepts.index') }}'">
                            Cancelar
                        </flux:button>
                        <flux:button variant="primary" color="sky" size="sm" type="submit">
                            @isset($concept)
                                Actualizar concepto
                            @else
                                Guardar concepto
                            @endisset
                        </flux:button>
                    </div>
                </form>
            </aside>
        </section>

    </div>
</x-layouts::app>
